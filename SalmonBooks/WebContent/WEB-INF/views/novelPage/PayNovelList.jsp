...
<body>
    ...
    <div class="novelContainer">
        <div id="hotNovelDiv" height="10%" style="background: white; margin: 10px 0 0 0;">
            <h3 class="textani"><span>조</span><span>회</span><span>수</span>&nbsp<span>T</span><span>O</span><span>P</span><span>3</span></h3>
        </div>
        <div class="hotnovelPanel">
            <ul class="hotNovelListUl">
                <c:forEach var="top3list" items="${top3list}">
               	<div class="hotframe">
                    <li>
                        <a href="selectOne.do?nno=${top3list.nno}" class="linkNovel">
                            <div class="novel_pic">
                                <img src="${top3list.img}">
                            </div>
                            <div class="novel_ingfo">
                                <p class="subj">${top3list.ntitle}&nbsp<span class="icon_up">유</span></p>
                                <p class="author"><span class="authorName" >${top3list.nWriter}</span><span class="num_total">&nbsp총
                                        ${top3list.total}화 |<span style="color : #FF6F61"> ${top3list.pay}P</span></span></p>
                                <%-- <p class="rating"><span class="score_area"><span
                                            class="icon_star">별점</span>&nbsp;<span class="counter">${top3list.rate}</span></span></p> --%>
                            </div>
                        </a>
                    </li>
               	</div>
    ...
    <div class="novelContainer">        
        <div id="uplNovelDiv" height="10%" style="background: white; margin: 10px 0 0 0;">
            <h3 class="textani"><span>업</span><span>로</span><span>드</span><span>된</span>&nbsp</span><span>유</span><span>료</span>&nbsp<span>웹</span><span>소</span><span>설</span></h3>
        </div>
        ...
        <div id="uplnovelPanel" class="main">
				...
				<c:set var="ntitlelen" value="${fn:length(ntitle)}"/>
					<div class="uplNovelLists__item uplnovelPanel">
						<li><a href="selectOne.do?nno=${list.nno}" class="linkNovel">
								...
								<span class="icon_up">유</span></p>
		                                <p class="author"><span class="authorName">${list.nWriter}</span><span class="num_total">&nbsp총
		                                        ${list.total}화 |<span style="color : #FF6F61"> ${list.pay}P</span></span></p><br>
		                                <!-- <p class="rating"><span class="score_area"><span
		                                            class="icon_star">별점</span>&nbsp;<span class="counter">0.0</span></span></p> -->
		                            </div>
		                        </a>
		                    </li>
		                </div>
			...
			<button  onclick="location.href='<%= request.getContextPath() %>/PayNovelList.do?currentPage=1&FREE=FALSE&COMPLATE=FALSE'"><<</button>
			<%  if(currentPage <= 1){  %>
			<button disabled><</button>
			<%  }else{ %>
			<button onclick="location.href='<%= request.getContextPath() %>/PayNovelList.do?currentPage=<%=currentPage - 1 %>&FREE=FALSE&COMPLATE=FALSE'"><</button>
			<%  } %>
			...
			<%      }else{ %>
				<button onclick="location.href='<%= request.getContextPath() %>/PayNovelList.do?currentPage=<%= p %>&FREE=FALSE&COMPLATE=FALSE'"><%= p %></button>
			<%      } %>
			<% } %>
			...
			<button onclick="location.href='<%= request.getContextPath() %>/PayNovelList.do?currentPage=<%=currentPage + 1 %>&FREE=FALSE&COMPLATE=FALSE'">></button>
			<%  } %>
			<button onclick="location.href='<%= request.getContextPath() %>/PayNovelList.do?currentPage=<%= maxPage %>&FREE=FALSE&COMPLATE=FALSE'">>></button>
